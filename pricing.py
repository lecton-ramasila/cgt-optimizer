...
from config import DEFAULT_EUR_USD, FX_TICKER


class PriceFetcher:
    def __init__(self, tickers: Iterable[str], fx_ticker: str = FX_TICKER, fallback_eur_usd: float = DEFAULT_EUR_USD) -> None:
        self.tickers = list(tickers)
        self.fx_ticker = fx_ticker
        self.fallback_eur_usd = fallback_eur_usd

    def fetch(self) -> tuple[dict[str, float | None], float]:
